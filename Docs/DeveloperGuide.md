...
4. Создайте страницу справки (раздел ниже).
5. Проверьте в AutoCAD → коммиты по правилам → утверждение → слияние `git merge --no-ff` в master.

### Справка по F1
Курсор на кнопке команды и F1 открывают страницу этой команды.

1. Создайте `Docs/Commands/<Key>.md`, где `<Key>` — ключ команды (`CommandInfo.Key`, он же имя папки команды).
   Имя файла обязано совпадать с ключом: по нему собирается адрес.
2. Возьмите за образец любую существующую страницу. Разделы: заголовок с номером и названием;
   таблица «кнопка / имя команды / изменяет ли чертёж»; Назначение; Перед началом; Порядок работы;
   Запросы и параметры; Ограничения; Результат; «Возможные ситуации и что делать»; ссылка «Все команды».
   Текст пишется для работника в AutoCAD, а не для программиста: описание для разработчика остаётся
   в `.md` внутри папки команды.
3. Добавьте страницу в таблицу команд в `Docs/UserGuide.md`, а в описание разработчика — строку
   «Инструкция пользователя» со ссылкой на неё.
4. `HelpUrl` заполнять вручную не нужно: статический конструктор `CommandCatalog` собирает адрес
   из константы `HelpBaseUrl` и ключа команды. Пустой `HelpUrl` отключает справку у кнопки,
   неверный адрес — тоже, с предупреждением в командной строке.
5. Страница должна быть в ветке `master` на GitHub: до push по F1 откроется страница «404».

Как это работает: `RibbonBuilder` назначает кнопке подсказку `RibbonToolTip` — со строкой вместо объекта
F1 не работает вовсе, — задаёт `IsHelpEnabled` и `HelpSource`. Дальше адрес обрабатывает встроенная
справка AutoCAD и показывает страницу в своём окне.

Открыть страницу в браузере по умолчанию из плагина не получилось. Проверенный и отклонённый вариант:
подписка на `ComponentManager.ToolTipOpened` плюс перехват F1 в `Application.PreTranslateMessage`
с `Handled = true` и `Process.Start`. Событие до плагина не доходит: F1 при открытой подсказке ленты
обрабатывается внутри AdWindows, и окно справки AutoCAD открывается по-прежнему. Код перехвата удалён,
чтобы не держать в проекте то, что не работает. Если к вопросу возвращаться, следующие кандидаты —
`Autodesk.Windows.ComponentManager.ProcessHelp` и `IComponentHostService.InvokeHelp`.

Адрес ведёт на страницу blob в ветке `master` публичного репозитория: GitHub показывает Markdown
с оформлением и не требует входа. Если репозиторий станет закрытым, справку придётся класть
в установочный архив и открывать с диска.
...
## 8. Журнал ошибок
Исключение внутри команды перехватывает `CommandRunner`: пользователь видит короткое сообщение,
полный стек вызовов уходит в `%LOCALAPPDATA%\PanelKomplekt\PanelKomplekt.log` (`Core/PluginLog.cs`).
Файл дописывается, при размере больше 1 МБ предыдущий сохраняется рядом как `PanelKomplekt.log.old`.
Сам журнал никогда не роняет команду: ошибки записи подавляются, тогда сообщение говорит,
что подробности записать не удалось. Просить журнал у заказчика — самый быстрый способ разобрать сбой.
